...
{
  float2 C = make_float2((hitpos.x - cuboidMin.x) * inv_cellsize.x,
                         (hitpos.z - cuboidMin.z) * inv_cellsize.z);
  ...
  float3 t = (hitpos - cuboidMin) / (cuboidMax - cuboidMin);
...
RT_PROGRAM void intersect(int primIdx)
{
  ...
  float3 L = (ray.origin + tnear * ray.direction - cuboidMin) * inv_cellsize;
  ...
  // Step 6
  float far_u = (D.x>0.0f?Lu+1:Lu) * cellsize.x + cuboidMin.x;
  float far_v = (D.z>0.0f?Lv+1:Lv) * cellsize.z + cuboidMin.z;
  ...
  while(tnear < tfar){
    ...
    if(ymin <= datamax && ymax >= datamin){
      // Step 10
      float3 EC = (ray.origin + tnear * ray.direction - cuboidMin) * inv_cellsize - make_float3(Lu, 0.0f, Lv);
